...
public static class FindDeadCodeTool
{
    ...
    [McpServerTool(Name = "find_dead_code", ReadOnly = true, Idempotent = true, OpenWorld = false)]
    [Description("Finds potentially unreferenced types, methods, and properties that may be dead code. Skips generated code (obj/bin, *.g.cs, <auto-generated> headers) and types activated by standard .NET contracts (IHostedService, ControllerBase, PageModel, ComponentBase).")]
        ...
        List<DeadCodeEntry> entries, CancellationToken ct)
    {
        foreach (var tree in compilation.SyntaxTrees)
        {
            ...
            if (GeneratedCode.IsGeneratedPath(tree.FilePath))
                continue;

            var model = compilation.GetSemanticModel(tree);
            var root = await tree.GetRootAsync(ct);

            if (GeneratedCode.HasAutoGeneratedHeader(root))
                continue;
    ...
    private static bool IsEntryPointOrTest(ISymbol symbol)
    {
        ...
        return HasControllerAttribute(symbol) || ActivationRoots.IsFrameworkActivated(symbol);
    }
